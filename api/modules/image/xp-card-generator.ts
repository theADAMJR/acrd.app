...
export class XPCardGenerator extends ImageGenerator 
{
    ...
    private addUserText(context, canvas: Canvas) 
    {
        ...
        context.fillStyle = card.primary || '#ffffff';
        ...
        context.fillStyle = card.tertiary || '#99AAB5';
    ...
    private async addXPBar(context, canvas, member: MemberDocument)
    {
        ...
        context.fillStyle = card.secondary || '#45377C';
        context.font = '32px Roboto, sans-serif';
        ...
        context.fillStyle = card.primary || '#45377C';
        context.fillRect(position.x, position.y, canvas.width - sizeOffset - 1, height);
        context.fillStyle = card.primary || '#482F5D';
        ...
        context.fillText(info.exp, canvas.width / 2.5, canvas.height / 1.35);
        
        context.fillStyle = '#0F0F0F';        
        context.fillText(`/`, canvas.width / 2.5 + context.measureText(info.exp).width, canvas.height / 1.35);

        context.fillStyle = card.secondary || '#45377C';
        context.fillText(`${nextLevelEXP}XP`, canvas.width / 2.5 + context.measureText(`${info.exp}/`).width, canvas.height / 1.35);
        context.fillText(`LEVEL ${info.level}`, canvas.width / 2.5, canvas.height / 1.175);
    }
}
